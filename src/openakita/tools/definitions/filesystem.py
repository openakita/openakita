...
- 多行 python -c 自动修复""",
    ...
    },
    {
        ...
        "description": (
            ...
            "- LARGE content (a long article / 上万字 deliverable): do NOT cram it all "
            "into one write_file call — passing very large content as a single JSON "
            "argument risks being truncated in transit. Instead write the file in "
            "sections: first write_file with the opening section, then APPEND each "
            "subsequent section with edit_file (anchor on the last line). This keeps "
            "every tool argument small and the final file complete.\n"
            "- Uses UTF-8 encoding\n\n"
