...
try:
    ...
    raise ImportError(
        "pywinauto is required for UIAutomation. "
...
class UIAClient:
    ...
    def get_active_window(self) -> Optional[UIAElementWrapper]:
        """
        获取当前活动窗口
    ...
    def find_element(
        self,
        root: Optional[UIAElementWrapper] = None,
        ...
        try:
            if root:
                elem = search_root.child_window(**criteria)
            else:
                elem = search_root.window(**criteria)
    ...
    def find_all_elements(
        ...
        try:
            if root:
                ...
                elements = search_root.windows(**{k: v for k, v in criteria.items() if k != "depth"})
    ...
    def find_element_by_path(
        ...
        for criteria in path:
            if current:
                found = current.find_child(**criteria)
            else:
                found = self.find_element(**criteria)
...
# 全局实例
_uia_client: Optional[UIAClient] = None
