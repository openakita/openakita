"""
Prompt Retriever (v2) — Retrieve relevant fragments from the memory system

v2 changes:
- Delegates to RetrievalEngine.retrieve() (multi-path recall + reranking)
- Retains _get_core_memory() and simple version for backward compatibility
"""
...
def retrieve_memory(
    ...
    """
    Retrieve fragments from the memory system relevant to the query

    v2: Prefers RetrievalEngine, falls back to legacy path.
    """
    ...
    if query and query.strip():
        ...
        if related:
            search_type = "semantic match" if used_vector else "keyword match"
            lines.append(f"\n## Related Memories ({search_type})\n")
...
def retrieve_memory_simple(
    ...
    """Read MEMORY.md directly (without vector search)"""
    from openakita.memory.types import MEMORY_MD_MAX_CHARS, truncate_memory_md
