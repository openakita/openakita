...
### 多种方法均因同一环境问题失败
- 如果助手**已尝试多种不同方法**（≥3种），且全部因**相同的环境/基础设施问题**失败（如网络不通、代理故障、DNS 解析失败、所有外部服务均超时），且助手已**向用户解释了原因并提供替代方案**（如建议用户手动操作）→ **COMPLETED**（环境问题非助手可解决，继续重试只会浪费时间导致系统无响应）
- 判断关键：是否所有失败都指向同一个根因（网络/代理/环境），而非各自独立的可修复错误？
