...
3. 断点续传走专用的只读端点 ``GET /api/chat/resume?since_seq=N``：服务端
   从 ``ringbuffer.replay_from(N)`` 把 N+1..M 重发一遍，再 tail-poll 接 active
   stream，对客户端表现为"无缝续传"。
4. 客户端用 ``seenSequenceNums: Set<number>`` dedup，防止 active 流和
   replay 出现重叠时同一个 delta 被消费两次。

跨 turn 防回放（replay floor）
=============================

ringbuffer 与 ``seq`` 是 per-conversation、跨 turn 持续累积的。若不加约束，
一个滞后的 ``Last-Event-ID`` / ``since_seq``（停在上一 turn 的断点）会让
``replay_from`` 把**上一 turn 已完成的尾巴**（最终答复）回放进新 turn——表现
为"明明答完了，一问新问题旧回复又被刷出来顶在上面"。

为此 :class:`SSESession` 维护一条 ``_replay_floor``：``POST /api/chat`` 每开一
个新 turn 就调 :meth:`SSESession.begin_turn` 把 floor 推到当前 max seq；
:meth:`SSESession.replay_from` 永远把请求的 ``last_seq`` clamp 到 floor 之上。
于是无论 POST 门（理论上的 stale header）还是 resume 门（stale since_seq），都
不可能跨 turn 边界回放——这是架构层面的硬保证，而非客户端自觉不传 header。
POST 自身不做 replay：一条全新消息不是断点重连。
...
class SSESession:
    ...
    _seq: int = 0
    # Replay floor — replay never returns events at or below this seq. The
    # ringbuffer + ``_seq`` are per-conversation and persist across turns, so
    # without a floor ``replay_from`` would happily hand a reconnecting client
    # the *previous, completed* turn's tail (its final answer) — which then
    # renders on top of the user's brand-new question. ``begin_turn()`` advances
    ...
    def begin_turn(self) -> int:
        ...
        start a new turn — the same generator keeps producing events — so this
        must be called exactly once per turn, before the turn's first event.

        It advances :attr:`_replay_floor` to the current max seq. Because every
        ringbuffer append for a turn happens inside that turn's generator
        (the deferred drain at the generator's finally appends the tail before
        it returns) and turns are lifecycle-serialized, by the time the *next*
        turn calls ``begin_turn`` the previous turn's events are all at or below
        the new floor. ``replay_from`` then can never reach back across the
        boundary. The floor is monotonic — it only ever moves forward.
        """
        with self._lock:
    ...
    def replay_from(self, last_seq: int | None) -> list[SSEEvent]:
        ...
        - ``last_seq`` predating the current turn (``< replay_floor``) → clamped
          ...
          previous, completed turn's events into the current turn.
