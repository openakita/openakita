...
- **身份信息不是沟通偏好**：用户说"我是医生/程序员/老师"是在陈述职业身份，不是在表达 emotional_distance 偏好。"professional"是沟通风格值，不要因为用户的职业带有"专业"含义就归入此维度
- **自我介绍只提取 address_style**：用户说"我叫XX/叫我XX"只需提取 address_style，职业、年龄、所在地等属于身份事实，不是沟通偏好，不要提取

## 常见误判（不要犯）

- ✗ "我是医生" → emotional_distance=professional （错：这是职业，不是沟通风格偏好）
- ✗ "我在北京工作" → care_topics=北京 （错：这是身份信息，不是关心话题）
- ✗ "叫我小明，我是做开发的" → formality=casual （错：自我介绍不代表语气偏好）
- ✓ "叫我小明" → address_style=小明 （对：这是明确的称呼偏好）
- ✓ "你说话太正式了" → formality=casual （对：这是直接修正信号）"""
...
class TraitMiner:
    ...
    def _parse_trait_response(self, content: str) -> list["PersonaTrait"]:
        ...
        for item in data:
            ...
            # 过滤低置信度的弱推测（0.55 以下多为不可靠的隐式信号）
            if confidence < 0.55:
                logger.debug(
                    f"[TraitMiner] Low confidence {confidence:.2f}, "
                    f"skipping {dimension}={preference}"
                )
                continue
