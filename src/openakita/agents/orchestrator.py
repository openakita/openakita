...
class AgentOrchestrator:
    ...
    async def _dispatch(
        ...
        try:
            ...
            _user_cancelled = _main_agent is not None and getattr(
                _main_agent, "_task_cancelled", False
            )
    ...
    async def delegate_async(self, request: dict) -> str:
        """异步委托接口，支持并行编排器"""
        session = request.get("session")
        from_agent = request.get("from_agent", "unknown")
        to_agent = request.get("to_agent", request.get("agent_id", ""))
        message = request.get("message", "")

        if not to_agent:
            raise ValueError("Missing to_agent in delegate request")
