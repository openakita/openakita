"""
FastAPI HTTP API server for SeeAgent.

集成在 `seeagent serve` 中，提供：
- Chat (SSE streaming)
- Models list
- Health check
- Skills management
...
def _find_web_dist() -> Path | None:
    ...
    Search order:
    1. seeagent/web/ (pip wheel install & PyInstaller bundle)
...
def create_app(
    ...
    from seeagent import get_version_string
    ...
    from seeagent.config import settings as _settings
    ...
    async def shutdown(request: Request):
        """Gracefully shut down the SeeAgent service process.
    ...
    async def _startup_org_runtime():
        ...
        if hasattr(app.state, "org_runtime") and app.state.org_runtime:
            try:
                from seeagent.core.engine_bridge import to_engine
...
async def start_api_server(
    ...
    api_thread = threading.Thread(
        target=_api_thread, daemon=True, name="seeagent-api",
    )
    api_thread.start()
    ...
    from seeagent.core.engine_bridge import set_api_loop, set_engine_loop
