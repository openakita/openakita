...
if TYPE_CHECKING:
    from seeagent.channels import MessageGateway
    ...
    def _ensure_deps(self) -> None:
        ...
        try:
            if self._profile_store is None:
                from seeagent.agents.profile import ProfileStore
                from seeagent.config import settings

                self._profile_store = ProfileStore(settings.data_dir / "agents")

            if self._pool is None:
                from seeagent.agents.factory import AgentFactory, AgentInstancePool

                self._pool = AgentInstancePool(AgentFactory())

            if self._fallback is None:
                from seeagent.agents.fallback import FallbackResolver

                self._fallback = FallbackResolver(self._profile_store)

            if self._log_dir is None:
                from seeagent.config import settings as _s
...
def _build_work_summary(record: dict) -> str:
    ...
    """
    from seeagent.core.tool_executor import smart_truncate
