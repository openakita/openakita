...
def _collect_preset_referenced_skills() -> set[str]:
    ...
    try:
        from seeagent.agents.presets import SYSTEM_PRESETS
        skills: set[str] = set()
...
class Agent:
    ...
    # 现在改为使用 `seeagent.core.im_context` 中的 contextvars（协程隔离）。
    ...
    def notify_pools_skills_changed() -> None:
        ...
        """
        try:
            from seeagent.main import _desktop_pool, _orchestrator
