"""Reproduce the exact request OpenAkita sends via compiler_think fallback."""
...
def call_api(system, user_msg, enable_thinking, temperature=1.0):
    """Exact same request format as OpenAkita's OpenAI provider."""
    body = {
        "model": MODEL,
        "messages": [
...
def extract_intent(content):
    """Quick extract intent from YAML output."""
...
def run_test(label, system, enable_thinking, temperature):
    print(f"\n{'=' * 60}")
    print(f"{label}")
    print(f"  thinking={enable_thinking}, temperature={temperature}")
    print(f"{'=' * 60}")
    for msg in TEST_MESSAGES:
        r = call_api(system, msg, enable_thinking, temperature)
        if isinstance(r, str):
            print(f'  "{msg}" → {r}')
        else:
            intent_val = extract_intent(r["content"])
            has_think = "YES" if r["reasoning"] else "NO"
            print(
                f'  "{msg}" → intent={intent_val}  '
                f'(in={r["in_tokens"]}, out={r["out_tokens"]}, reasoning={has_think})'
            )


# Test matrix: exactly matching OpenAkita's parameters
run_test("OLD PROMPT + thinking=False + temp=1.0 (OpenAkita exact match)",
         OLD_SYSTEM, False, 1.0)

run_test("OLD PROMPT + thinking=False + temp=0.3 (lower randomness)",
         OLD_SYSTEM, False, 0.3)

run_test("NEW PROMPT + thinking=False + temp=1.0",
         NEW_SYSTEM, False, 1.0)

run_test("NEW PROMPT + thinking=False + temp=0.3",
         NEW_SYSTEM, False, 0.3)
