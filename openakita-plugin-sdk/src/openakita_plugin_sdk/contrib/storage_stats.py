...
Replaces the synchronous `Path.rglob()` pattern used in seedance-video /
tongyi-image which blocks the event loop and walks unbounded trees.

Audit3 N1.x: large media folders can stall the UI for seconds — wrap with
``asyncio.to_thread`` and respect a hard ``max_files`` cap.
"""
...
async def collect_storage_stats(
    ...
    *,
    ...
    Args:
        roots: A single dir or list/tuple of dirs.  Missing dirs are silently
            skipped.
        max_files: Hard cap; once reached, ``truncated=True`` and we return
            early so the UI never stalls on huge trees.
        sample_paths: How many absolute file paths to include in
            ``sampled_paths`` for debugging.
        skip_hidden: Skip dotfiles / dotdirs (default: True).
    """
    roots_list = (
