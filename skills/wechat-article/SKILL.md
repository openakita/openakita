...
# WeChat Official Account文章发布助手

Designed for微信公众号内容创作和发布打造的技能，Provides从写作到排版到发布的全流程Supports，包括 Markdown 转换、富文本样式、封面设计和摘要Generation。

## When to Use

- 撰写公众号原创文章（深度长文、观点评论、行业Analyze）
- Markdown 文章转微信公众号格式
- 文章排版美化和样式优化
- 封面图选择和摘要Generation
...
微信公众号Edit器不Supports标准 Markdown，需要将 Markdown 转为内联样式的 HTML。关键限制：

- **不Supports** `<style>` 标签和外部 CSS
- **不Supports** `class` 属性
- **所有样式必须内联**（`style="..."` 属性）
- 图片必须Upload到微信素材库（外链图片无法Display）
- 不Supports JavaScript
...
| `[链接](url)` | `<a style="color:#576b95;text-decoration:none;">链接文字</a>`（微信不Supports外链，仅Used for阅读原文） |
...
| Parameter | Recommendations值 | Description |
|------|--------|------|
| 正文字号 | 15px | 移动端最佳阅读大小 |
| 行间距 | 1.8 | 阅读舒适度最佳 |
| 字间距 | 0.5px | 增加空气感 |
...
公众号摘要Display在推送消息和分享卡片中，最多 **120 字**。

**摘要写作原则：**
- 第一句概括文章核心价值
- Includes 1-2 个关键词（SEO）
- 制造悬念或给出利益点
- 避免重复标题内容

**模板：**
```
[核心观点/发现]。本文从[角度1]、[角度2]等方面深入Analyze，
并Provides[具体价值]。适合[目标读者]阅读。
```

## 发布工作流

### 方式一：API Automatic发布（Recommendations）

适Used for有微信公众号开发者权限的用户。

```
Step 1: Markdown 文章 → 微信 HTML 格式转换
  ↓
Step 2: 图片资源Upload至微信素材库
  - Call素材Manage接口Upload图片
  - Get media_id 替换图片链接
  ↓
Step 3: Generation封面图 + 摘要
  ↓
Step 4: Create图文素材（draft）
...
| 接口 | 用途 |
|------|------|
| `POST /cgi-bin/material/add_material` | Upload永久素材 |
| `POST /cgi-bin/media/uploadimg` | Upload文章内图片 |
| `POST /cgi-bin/draft/add` | 新建草稿 |
| `POST /cgi-bin/draft/update` | Update草稿 |
| `POST /cgi-bin/freepublish/submit` | 发布 |
| `POST /cgi-bin/message/mass/preview` | 预览 |

### 方式二：Manual发布

适Used for无 API 权限或偶尔发布的用户。

```
Step 1: Generation微信 HTML 格式内容
  ↓
Step 2: 复制 HTML 到微信公众号后台Edit器
  - Open mp.weixin.qq.com → 内容Manage → 新建图文
  - Switch到「代码Edit」模式（</>按钮）
  - 粘贴 HTML 代码
  ↓
Step 3: 切回「可视化Edit」模式检查排版
  - 确认标题样式正确
  - 确认图片正常Display
  - 确认引用框和分隔线样式
  ↓
Step 4: Upload封面图、填写摘要
  ↓
Step 5: 预览 → 手机端检查 → 发布
```

**Manual发布注意事项：**
- HTML 中的图片需提前Upload到微信素材库
- 外链图片不会Display（微信安全策略）
- 粘贴后检查YesNo有样式丢失
- 建议先在手机端预览再发布
...
> 性能不Yes锦上添花，而Yes用户体验的基石。

## 为什么性能很重要

根据 Google 的研究，页面Load时间超过 **3秒**，
53% 的移动用户会离开。
```
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 图片不Display | Use了外链图片 | Upload到微信素材库，Use微信图片链接 |
| 样式丢失 | Use了 class 或外部 CSS | 所有样式改为内联 style 属性 |
| 排版错乱 | HTML 标签嵌套错误 | 检查标签闭合，简化嵌套层级 |
| 链接无法Click | 微信限制外链 | Use「阅读原文」放置唯一外链 |
| 代码块不美观 | 微信不Supports语法高亮 | 用背景色块+内联样式模拟代码块 |
| 表格Display不全 | 宽表格溢出 | 控制列数，或用列表替代表格 |
...
- 字数控制在 **15-30 字**
- Includes核心关键词
- Use数字增加具体感（`7个方法`、`3分钟学会`）
- 适当Use竖线分隔（`前端优化 | 你必须知道的 7 个技巧`）

### 开头黄金三行

读者在 3 秒内决定YesNo继续阅读，开头必须有冲击力：
