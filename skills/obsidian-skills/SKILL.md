...
```markdown
[[笔记名称]]                    # 链接到同名笔记
[[笔记名称|Display文本]]            # 自定义Display文本
[[笔记名称#标题]]               # 链接到特定标题
[[笔记名称#^block-id]]          # 链接到特定块
[[笔记名称#标题|Display文本]]       # 标题链接 + 自定义文本
```
...
**折叠 Callout：**
```markdown
> [!tip]- Click展开       # Default折叠
> 隐藏内容

> [!tip]+ Click折叠       # Default展开
> 可折叠内容
```
...
```markdown
%%这Yes Obsidian 注释，不会在预览模式中Display%%
...
2. **动态Update** — 新建笔记后检查YesNo需要加入相关 MOC
...
**行内查询：**
```markdown
今天Yes `= date(today)`，本库共有 `= length(file.lists)` 条列表项。
```

### Dataview 友好的属性设计

- Use英文属性名（中文属性名在某些查询中可能出问题）
- 日期Use `YYYY-MM-DD` 格式
- 布尔值Use `true`/`false`
- 列表属性Use YAML 数组语法
...
Obsidian Canvas Yes可视化组织笔记关系的画布工具。

### Canvas files格式

Canvas files为 `.canvas` 后缀的 JSON 文件，Includes节点（nodes）和边（edges）。
...
### Create Canvas 时的注意事项

- 节点坐标以画布中心为原点
- 建议节点宽度 200–400px
- Use `group` 节点对相关卡片分组
- 边可以Includes `label` 描述关系

---

## Bases Supports

Obsidian Bases Yes Obsidian 1.8+ 引入的内置数据库视图功能。

### Bases 概述

- Bases 文件以 `.base` 为后缀，存储在 Vault 中
- Automatic从笔记的 frontmatter properties Extract结构化数据
- Supports表格视图、筛选、排序、分组
- 无需 Dataview 插件即可实现结构化查询

### Create Bases 时的建议

1. **统一属性** — 同一类型的笔记Use相同的 frontmatter 属性集
2. **属性类型** — 在 Obsidian Set中预定义属性类型，确保数据一致性
3. **筛选条件** — Use文件夹或标签限定数据来源
...
1. **询问Save位置** — 始终先确认用户期望的文件夹路径
2. **选择或Create模板** — 根据笔记类型Use对应模板
3. **填写 frontmatter** — 确保所有必需属性Full
4. **编写内容** — Use OFM 语法
5. **建立链接** — 添加 wikilinks 连接相关笔记
6. **Update MOC** — 如有对应 MOC，将新笔记加入其中
...
- **不要自行决定Save路径** — 永远先问用户
- **保持笔记原子性** — 一条笔记一个核心概念
- **优先Use wikilinks** — 而非标准 Markdown 链接
- **YAML frontmatter 必须Yes文件第一行** — 前面不能有空行
- **属性名Use英文** — 确保 Dataview 和 Bases 兼容
- **图片等附件放在专用文件夹** — 如 `Attachments/`
- **定期维护链接** — 重命名笔记后检查链接YesNoAutomaticUpdate
- **备份 Vault** — 建议Use Git 或 Obsidian Sync
