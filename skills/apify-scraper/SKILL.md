...
- When the user needs从网站抓取结构化数据（商品信息、社交媒体帖子、search结果等）
- 需要批量get社交媒体平台数据（Instagram、TikTok、YouTube 等）
- 需要抓取 Google search结果、地图信息、评价数据
- 需要定期监控网页变化
- 需要将非结构化网页内容转换为 JSON/CSV
- 需要从电商平台Extract商品和价格信息
...
| 配置项 | Description |
|--------|------|
| `APIFY_TOKEN` | Apify API Token，在 https://console.apify.com/account/integrations get |
...
| `pandas` | 数据handle与导出 | `pip install pandas` |
...
Apify 平台上有数千个 Actor（即预构建的爬虫/Automatic化程序）。本技能聚焦 55+ 个经过验证的、面向 AI 数据Extract优化的 Actor。
...
| Twitter/X | Scraper | `apidojo/tweet-scraper` | 推文search |
...
| Google | Trends | `emastra/google-trends-scraper` | search趋势 |
| Google | News | `lhotanova/google-news-scraper` | 新闻search |
| Google | Shopping | `epctex/google-shopping-scraper` | 商品价格 |
| Bing | Search | `nicefellow/bing-search-scraper` | Bing search结果 |
...
| eBay | Scraper | `drobnikj/ebay-scraper` | 商品search |
...
|------|-------|----------|---------|
| 网页抓取 | Web Scraper | `apify/web-scraper` | 通用网页数据Extract |
| 网页截图 | Screenshot | `apify/screenshot-url` | 网页截图 |
| 链接Extract | Link Extractor | `apify/link-extractor` | 页面链Receive集 |
| RSS 解析 | RSS Feed | `drobnikj/rss-feed-reader` | RSS 源数据 |
| AI Extract | GPT Scraper | `drobnikj/gpt-scraper` | AI 驱动智能Extract |

### Actor Automatic选择策略

Agent 根据用户需求Automatic选择最合适的 Actor：

1. **解析用户意图** — 从用户描述中识别目标平台和数据类型
2. **匹配 Actor** — 根据平台和功能匹配上方表格
3. **若无精确匹配** — Use通用 Web Scraper 或 GPT Scraper
4. **确认方案** — 向用户确认将Use的 Actor 和预计数据量
...
将数据转换为When the user needs的格式（JSON/CSV/表格摘要）。

---

### Workflow 2: search引擎数据抓取
...
### Workflow 3: 通用网页数据Extract

当没有专用 Actor 时，Use AI 驱动的通用Extract：

**方法 A — Web Scraper（Based on选择器）**
...
**症状**：所有请求Returns 401
**解决**：确认 `.env` 中的 `APIFY_TOKEN` 已正确Set
...
**症状**：Returns 403 或空结果
**解决**：
- Use Apify 的代理池（Actor 通常自带）
- 降低并发和频率
- 增加请求间隔

### 4. 数据格式不一致

不同 Actor Returns的数据结构不同。在handle数据前先检查字段：
...
4. **频率限制** — 不对目标网站造成过大负载
5. **数据用途** — 仅Used for合法目的（Analyze、研究、商业决策）

### 禁止行为

1. 抓取个人隐私信息Used for骚扰或监控
2. 大量抓取导致目标网站服务降级
3. 绕过付费墙或版权保护
4. 出售未经授权的第三方数据
5. Used for垃圾信息、虚假评论等恶意目的

### 建议做法

1. 缓存已get数据，避免重复抓取
2. Use增量抓取而非全量抓取
3. 在非高峰时段Execute大规模任务
4. Provides User-Agent 标识你的爬虫身份
5. 记录抓取日志以便审计
