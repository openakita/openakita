...
# Generate AGENTS.md — 项目开发规范Generation器

> **AGENTS.md** Yes AI 编码 Agent 的行业标准项目指引文件（[agents.md](https://agents.md/)），
> 被 Cursor、Codex、Copilot、Jules、Windsurf、Aider、opencode 等 20+ 工具Supports。
> 一份文件，所有 AI 工具通用。

## When to Use

- 用户说"Generation AGENTS.md"、"初始化项目规范"、"create项目指引"
...
7. **现有 AGENTS.md**：如果已存在，Read后在其基础上update

### Step 2: Generation AGENTS.md

按以下模板结构Generation，**只写与项目相关的段落**，省略不适用的Partial：
...
Use `write_file` Write到项目根目录的 `AGENTS.md`。

### Step 4: Monorepo 检查

如果检测到 monorepo 结构（`apps/`、`packages/`、`services/` 等子项目），**询问用户**YesNo需要为子项目也Generation嵌套的 AGENTS.md。子项目的 AGENTS.md 只写该子项目特有的内容，不重复根级内容。

## Important Rules

- **控制长度**：AGENTS.md 建议 150 行以内，不要写成Full文档
- **只写有用的**：不要填充模板中每个段落，省略不适用的
- **面向 AI Agent**：内容Yes给 AI 看的，不需要"入门教程"级别的解释
- **可Execute的命令**：构建、测试、lint 等必须Yes可以直接复制Execute的命令
- **不要暴露敏感信息**：不要写 API key、密码、内部 URL 等
- **中英文皆可**：跟随项目的主要语言（README Yes中文就用中文）

## Examples

用户说："帮我Generation AGENTS.md"

→ Execute Step 1-4，扫描项目后Generation文件。

用户说："update一下 AGENTS.md"

→ 先Read现有 AGENTS.md，结合项目current statusupdate过时的内容。
