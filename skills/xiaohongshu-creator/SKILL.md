...
Designed for小红书平台打造的内容创作技能，帮助你Generation符合平台调性的高质量笔记，涵盖标题、正文、话题标签和配图建议。

## When to Use

- 撰写种草笔记（好物Recommendations、购物分享）
- 撰写产品测评笔记
...
| 权威法 | 借势专业背书 | `皮肤科医生Recommendations的面霜💊` |
| 场景法 | 具体Use场景 | `约会前30分钟急救妆容💄` |
...
| 类别 | 数量 | Description | Example |
|------|------|------|------|
| 核心词 | 2 个 | 笔记主题精准关键词 | `#面膜Recommendations` `#保湿面膜` |
| 品类词 | 2 个 | 所属品类/领域 | `#护肤` `#美妆好物` |
| 场景词 | 2 个 | Use场景/人群 | `#学生党护肤` `#换季护肤` |
| 热门词 | 2 个 | 平台热门话题 | `#好物分享` `#我的爱用物` |

**选择原则：**
- 优先选择Search量大但竞争适中的标签
- 避免过于宽泛的标签（如 `#生活`）
- Includes长尾关键词提升Search曝光
- 关注平台当前热门话题榜

### 四、配图风格建议

小红书Yes视觉驱动平台，封面决定 80% 的Click率。

**10 种Recommendations视觉风格：**

| 编号 | 风格 | 适用类型 | 要点 |
|------|------|---------|------|
| 1 | 对比拼图 | 测评/效果展示 | 左右或上下对比，标注差异 |
| 2 | 清单图 | 好物合集/Recommendations | 白底九宫格产品陈列 |
...
输入 → 产品名称、品类、价格、目标人群
  ↓
Step 1: Generation 3 个标题方案（痛点法/数字法/场景法各一）
  ↓
Step 2: 撰写正文
  - Hook：个人Use感受/发现契机
  - Core：产品亮点（3-5个）、Use方法、适合人群
  - Summary：一句话总结Recommendations理由
  - CTA：引导收藏和讨论
  ↓
Step 3: Generation 8 个话题标签
  ↓
Step 4: 封面建议（Recommendations风格 9 实拍特写 或 风格 2 清单图）
  ↓
输出 → Full笔记（可直接发布）
```
...
Step 2: 撰写正文
  - Hook：测评动机/痛点引入
  - Core：逐项对比（成分/价格/Use感/性价比）
  - Summary：各产品评分或排名
  - CTA：`你们用过哪个？评论区聊聊`
  ↓
Step 3: 话题标签（增加品牌词标签）
  ↓
Step 4: 封面建议（Recommendations风格 1 对比拼图 或 风格 6 数据图表）
  ↓
输出 → Full测评笔记
```
...
Step 3: 话题标签（增加 `#教程` `#手把手教学` 等）
  ↓
Step 4: 封面建议（Recommendations风格 3 步骤图 或 风格 4 文字封面）
  ↓
输出 → Full教程笔记 + 多图建议（每步一张图）
```
...
Step 3: 话题标签（增加情感/生活方式标签）
  ↓
Step 4: 封面建议（Recommendations风格 5 场景氛围图 或 风格 8 Vlog截图）
  ↓
输出 → Full分享笔记
```
...
**用户输入：** `帮我写一篇关于珂润面霜的种草笔记，目标人群Yes敏感肌学生党`
...
**话题标签：**
`#珂润面霜` `#敏感肌护肤` `#学生党护肤` `#平价面霜Recommendations` `#神经酰胺` `#换季护肤` `#好物分享` `#护肤干货`
...
### 封面建议
- Recommendations风格：[编号+名称]
- 具体建议：[详细说明]
- 配色方案：[色系建议]

### 发布建议
- Recommendations时段：[具体时间]
- 注意事项：[补充说明]
```
