...
高德地图综合服务，包括地点Search、路径规划、旅游规划和数据可视化等功能。

## 首次配置

访问高德开放平台 https://lbs.amap.com/api/webservice/create-project-and-key Create应用并Get Key。
Set环境变量：export AMAP_WEBSERVICE_KEY=your_key
...
Supports：walking（步行）、driving（驾车）、riding（骑行）、transfer（公交）
...
高德地图 Web 服务 Python 封装，需Set AMAP_WEBSERVICE_KEY。
