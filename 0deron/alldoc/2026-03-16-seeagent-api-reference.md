...
- [2. 对话模块 (Chat)](#2-对话模块-chat)
  - [2.1 Desktop Chat API](#21-desktop-chat-api)
  - [2.2 SeeCrab Chat API](#22-seecrab-chat-api)
  - [2.3 SSE 事件类型](#23-sse-事件类型)
...
- [附录 B: 前端 API Client 参考](#附录-b-前端-api-client-参考)
- [附录 C: Pydantic 数据模型](#附录-c-pydantic-数据模型)
...
### 2.2 SeeCrab Chat API

> **路由前缀**: `/api/seecrab`
> **源码**: `src/seeagent/api/routes/seecrab.py`

SeeCrab 是独立的轻量前端，使用 SeeCrabAdapter 转换事件格式。

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/seecrab/chat` | **SSE 流式** SeeCrab 聊天端点 |
| GET | `/api/seecrab/sessions` | 列出会话 |
| GET | `/api/seecrab/sessions/{session_id}` | 获取会话详情和消息历史 |
| POST | `/api/seecrab/sessions` | 创建新会话 |
| PATCH | `/api/seecrab/sessions/{session_id}` | 更新会话元数据（标题等） |
| DELETE | `/api/seecrab/sessions/{session_id}` | 删除会话 |
| POST | `/api/seecrab/answer` | 回复 ask_user 事件 |

#### POST `/api/seecrab/chat` (SSE Streaming)
...
**SSE 事件流（SeeCrab 格式）:**
```
data: {"type": "session_title", "session_id": "seecrab_abc123", "title": "你好"}
data: {"type": "thinking", "content": "..."}
data: {"type": "plan_checklist", "steps": [...]}
data: {"type": "step_card", "stepId": "...", "title": "...", "status": "running"}
data: {"type": "ai_text", "content": "你好！有什么可以帮你的？"}
data: {"type": "ask_user", "question": "请选择一个选项", "options": [...]}
data: {"type": "agent_header", ...}
data: {"type": "artifact", "artifact_type": "file", "file_url": "/api/uploads/..."}
data: {"type": "timer_update", ...}
data: {"type": "done"}
```

**Demo (JavaScript - 前端 SSEClient):**
```javascript
import { sseClient } from '@/api/sse-client'

// 发送消息并接收 SSE 流
await sseClient.sendMessage('帮我分析一下代码', 'session_001', {
  thinking_mode: 'auto',
})
...
| 事件类型 | 说明 | 关键字段 |
|----------|------|----------|
| `session_title` | 会话标题（首条消息自动生成） | `session_id`, `title` |
| `thinking` | 思考内容 | `content` |
| `plan_checklist` | 计划清单 | `steps` |
| `step_card` | 工具/技能执行卡片 | `stepId`, `title`, `status`, `input`, `output` |
| `ai_text` | AI 文本回复 | `content` |
| `ask_user` | 用户确认提问 | `question`, `options` |
| `agent_header` | Agent 标识头 | - |
| `artifact` | 产物 | `artifact_type`, `file_url` |
| `timer_update` | 计时更新 | - |
| `heartbeat` | 心跳 | - |
| `done` | 完成 | - |
...
export const httpClient = {
  // 列出会话
  listSessions: () => GET<{ sessions: any[] }>('/sessions'),

  // 创建会话
  createSession: () => POST<{ session_id: string }>('/sessions'),

  // 获取会话（含消息历史）
  getSession: (id: string) => GET<{ session_id: string; title: string; messages: any[] }>(`/sessions/${id}`),

  // 更新会话标题
  updateSession: (id: string, data: { title?: string }) => PATCH(`/sessions/${id}`, data),

  // 删除会话
  deleteSession: (id: string) => DELETE(`/sessions/${id}`),

  // 提交用户回答
  submitAnswer: (conversationId: string, answer: string) =>
    POST('/answer', { conversation_id: conversationId, answer }),
}
```
...
```javascript
// 开发环境代理：/api/* → http://127.0.0.1:18900
server: {
  port: 5174,
  proxy: {
    '/api': { target: 'http://127.0.0.1:18900', changeOrigin: true },
  },
}
```
...
> **源码**: `src/seeagent/api/schemas.py`, `src/seeagent/api/schemas_seecrab.py`
...
> **总端点数**: 120+
> **SSE 流式端点**: 3 个 (`/api/chat`, `/api/seecrab/chat`, `/api/orgs/{id}/subscribe`)
> **WebSocket 端点**: 1 个 (`/ws/events`)
> **路由模块文件**: 24 个 Python 模块 (`src/seeagent/api/routes/`)
> **生成日期**: 2026-03-16
