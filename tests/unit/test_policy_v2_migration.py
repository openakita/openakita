...
class TestV113TrustDefaultBcSurface:
    """v1.27.13 把 schema 默认从 protect/default 切到 trust/trust 后，
    把 migration 这层会受影响的 BC surface 钉住，避免未来无声扭曲。

    重要不变量：
    - migration **不**显式注入 ``profile.current`` / ``confirmation.mode``，
      留给 ``PolicyConfigV2.model_validate`` 用 schema 默认填充。
    - 这意味着 v1 YAML 无 confirmation 块的用户升级后会从 default → trust，
      属于"已接受的 BC"。本测试只是把这条路径**可见地**写下来，未来如要
      额外保留 v1 行为只需改 migration，不需要回退 schema 默认。
    - 已显式写 v1 别名的用户继续受 LEGACY_MODE_ALIASES 翻译保护。
    """

    def test_v1_yaml_without_confirmation_block_falls_to_schema_default(self) -> None:
        """No confirmation block → migration leaves it alone → loader fills
        schema default = trust (v1.27.13+).
        """
        from openakita.core.policy_v2 import PolicyConfigV2
        ...
        v1 = {
            "security": {
                "enabled": True,
                "zones": {"workspace": ["${CWD}"]},
                "command_patterns": {"custom_critical": []},
            }
        }
        v2_dict, report = migrate_v1_to_v2(v1)
        # Migration 不主动塞 confirmation —— 没有 source v1 字段就什么都不写
        assert "confirmation" not in v2_dict["security"]
        # 经过 schema 校验后，落到 v1.27.13+ 的出厂默认 trust
        cfg = PolicyConfigV2.model_validate(v2_dict["security"])
    ...
    def test_v1_yaml_with_explicit_smart_still_maps_to_default(self) -> None:
        """已显式写 v1 ``smart`` 的用户走 alias 翻译，不被 schema 默认影响。"""
        from openakita.core.policy_v2 import PolicyConfigV2

        v1 = {"security": {"confirmation": {"mode": "smart"}}}
        v2_dict, _ = migrate_v1_to_v2(v1)
        assert v2_dict["security"]["confirmation"]["mode"] == "default"
        cfg = PolicyConfigV2.model_validate(v2_dict["security"])
        # 显式 smart → default，不被新出厂默认 trust 抹掉
        assert cfg.confirmation.mode == "default"

    def test_v1_yaml_with_explicit_yolo_maps_to_trust(self) -> None:
        from openakita.core.policy_v2 import PolicyConfigV2

        v1 = {"security": {"confirmation": {"mode": "yolo"}}}
        v2_dict, _ = migrate_v1_to_v2(v1)
        assert v2_dict["security"]["confirmation"]["mode"] == "trust"
        cfg = PolicyConfigV2.model_validate(v2_dict["security"])
        assert cfg.confirmation.mode == "trust"

    def test_v1_yaml_with_explicit_cautious_maps_to_strict(self) -> None:
        from openakita.core.policy_v2 import PolicyConfigV2

        v1 = {"security": {"confirmation": {"mode": "cautious"}}}
        v2_dict, _ = migrate_v1_to_v2(v1)
        assert v2_dict["security"]["confirmation"]["mode"] == "strict"
        cfg = PolicyConfigV2.model_validate(v2_dict["security"])
        # 显式 cautious → strict，不被新出厂默认 trust 抹掉
        assert cfg.confirmation.mode == "strict"
